#!/usr/bin/env bash
# 在 Ubuntu 16.04/glibc 2.23 基线上完成 PyInstaller 与 StaticX 全链构建。
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive

apt-get update
apt-get install -y --no-install-recommends ca-certificates wget bzip2 binutils patchelf file build-essential musl-tools
...
PACK_PYTHON="$CONDA_ROOT/bin/python" PACK_SKIP_FRONTEND_BUILD=1 PACK_STATICX_SOURCE_BUILD=1 ./tools/pack.sh
