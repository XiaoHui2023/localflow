...
When configuration resolution is invalid, its problem list and resolved-tree projection become the primary work surface. Stretch the debug card through the remaining workbench height instead of sizing it from its current content or imposing a fixed results maximum. Keep the header fixed and make the problem and result regions independently scrollable: columns on a sufficiently wide workbench, bounded vertical regions when compact. A short error list must not collapse the whole debug surface and leave an unused lower half.
...
Diagnosis/submit parity is representation parity: validate common task fields and plugin-declared fields after includes and variables resolve, because a syntactically valid placeholder can become `null`, a list with a null member, or another incompatible value. The run surface must report that resolved path before submission. Optional common fields explicitly set to null are absence, while unrelated site-owned null keys remain legal and available to plugins.
...
Record run history on the server in the same accepted batch transaction so refreshes and different browsers see truthful usage rather than click telemetry. Keep favorites browser-local until the product has user identity and preference synchronization; cap them at 100. The current configuration owns one star action beside Edit/Run/Save with stable pressed semantics and accessible `收藏配置`/`取消收藏` names. An in-app move/rename has an authoritative source/target pair and remaps server history plus browser favorites. An external move has no reliable identity key; never guess from basename or content. Remove its stale favorite during workspace synchronization. Also treat a 404/410 between rendering and opening a quick entry as expected invalidation: remove the favorite, resynchronize, and leave the current workbench intact rather than surfacing an unhandled rejection. The local tablist exists inside the configuration panel, swaps only the explorer rail, and keeps the workbench mounted. Collapsing configuration hides the entire local control without moving it into global navigation.
...
- A command configuration whose resolved label list contains null is non-runnable in diagnosis with a `labels.0` error; clicking Run is never required to discover it. Null optional common fields expand to their `TaskCreate` defaults, while an unrelated custom null remains accepted.
- The invalid-configuration debug card occupies more than 75% of the complete workbench height at the gated desktop viewport, and its body occupies more than 75% of the card; problem/result overflow scrolls inside the allocated regions rather than shortening the primary surface.
