...
| Full command | Proves the exact shell text or argv | Keep |
...
| Labels and operator text | Lets the operator review/copy metadata | Keep; no status decoration |
...
| Copy feedback | Confirms an invisible clipboard side effect | Keep in the original control: Copy icon to Check, pressed surface, no toast or layout movement |
...
| In-place icon/state transition | Strong local causality and stable geometry | Needs reduced-motion handling | Selected |
...
- Copy tests observe the persistent affordance and copied clipboard value; CSS keeps dimensions fixed and disables motion under reduced-motion preference.
