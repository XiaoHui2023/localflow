...
| --- | --- | --- | --- |
| Lifecycle | Task `state` and plugin status label | Visible `运行中`/`历史` groups plus per-row state text; active/warning/danger color is redundant | Changes only with task lifecycle |
...
Active terminals sort before retained history; each group sorts newest first. Successful history stays neutral. Starting/running names use the accent, stopping uses warning, and failed/lost status uses danger, but text/grouping always conveys the same state without color.

## Rejected alternatives

- A state-colored dot on every row: ambiguous, visually noisy, and color-only.
- A completion dot: duplicates the visible terminal state and collides with “new output after completion.”
...
Edge opens a live terminal and sees no dot merely because it is running. It selects a retained history terminal; when the live task's real log size grows, that background row gains exactly one accessible marker. Selecting the live row clears it. The same journey proves “运行中” precedes “历史,” normal lifecycle text remains present, terminal bytes contain no connection/replay notices, and wide/mobile layouts do not overflow.
