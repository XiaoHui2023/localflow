...
- A cross-platform test for automatic login-Shell selection must compare against the detector contract, not a literal `/bin/sh`. Windows can legitimately exercise the final fallback while a hosted Ubuntu account resolves to `/bin/bash`; hard-coding the fallback creates a local-pass/CI-fail oracle even when the product behavior is correct.
- A shared SQLite connection with `check_same_thread=False` still needs one application lock around both transaction bodies and result decoding. In particular, an idempotency hit must finish its read transaction once, keep the lock while materializing existing task rows, and guard exception rollback with `Connection.in_transaction`; otherwise a concurrent post-commit read can observe a row while another thread changes the same connection's transaction state.
