...
- Inspection must show every consequential resolved value, including command, working directory, case/simulation paths, labels, mutex keys and configured output/log paths. Check only inputs that must already exist. Do not reject or warn merely because an output or log file is expected to be created by the forthcoming run.
