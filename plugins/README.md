...
配置必须在顶层声明 `plugin`。新增插件应提供 `config_model`、`run_fields`、状态映射以及可直接提交的 `example`；需要本次运行输入时提供 `api_inputs`。`run_fields` 只公开用户每次运行确实需要改变的值，命令、工作目录和 Case 目录等稳定内容留在配置文件中。
