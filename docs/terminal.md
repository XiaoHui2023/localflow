...
任务进入终态后不会从终端任务列表消失。同一个 xterm 实例从日志偏移 0 只读回放保留期内的终端历史，标题区显示“只读历史”，输入框、Ctrl+C 与 Ctrl+D 全部移除；即使绕过网页，服务端也因任务状态拒绝写入和 resize。连接、断开、回放和只读状态只属于网页标题与可访问状态，不通过 xterm 写入，因此复制、查找、字节偏移和历史记录只包含任务日志。任务详情与终端输出共用保留和容量合同，历史日志被空间保护回收后不能宣称仍完整可用。
