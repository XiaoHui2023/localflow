...
任务显式配置 `source` 时，执行器在 `process.command` 前为每个冻结后的绝对路径写一条 `process.source` 生命周期记录。它属于真实执行输入而非界面提示，因此实时与历史终端都可查看、搜索和复制；内部 Shell、`-ic` 与自动 `cd` 包装仍不写入用户命令显示。
...
xterm 选区保持原生终端语义：双击或拖动选择后，Ctrl/Cmd+C 复制所选字节；存在选区时才临时显示“复制选中”动作，作为浏览器快捷键被系统或远程桌面截获时的可发现回退。实时终端没有选区时 Ctrl+C 仍发送控制字符；历史终端只复制、永不向进程发送输入。查找浮层必须先声明不透明的主题背景，再以 `color-mix()` 增强，旧浏览器不支持该函数时也不能退化为透明背景。
