...
任务进入终态后不会从终端任务列表消失。同一个 xterm 只读回放保留期内的终端历史，默认装载末尾至多 4 MiB，并可前后切换固定字节窗；标题区显示“只读历史”，输入框、Ctrl+C 与 Ctrl+D 全部移除。完整日志检索由服务端按固定块流式完成，结果携带字节偏移，点击后只加载命中附近窗口；因此 1 GB 级日志不会一次进入浏览器或 Python 堆。即使绕过网页，服务端也因任务状态拒绝写入和 resize。连接、断开、回放和只读状态只属于网页标题与可访问状态，不通过 xterm 写入，因此复制、查找、字节偏移和历史记录只包含任务日志。任务详情与终端输出共用保留和容量合同，历史日志被空间保护回收后不能宣称仍完整可用。

## 终端列表状态方案

左侧列表采用事件与生命周期分离的两层语义。“运行中”组始终位于“历史”组之前，两组内部按最近时间倒序；每项同时显示任务名、标签和状态文字，同名任务可按标签区分，名称颜色只是状态增强。普通、完成和很久没有输出的条目都不显示圆点。任务列表 API 对每次读取实时投影当前日志文件大小；只有未选中终端的 `log_size` 在当前页面会话中真实增长，才显示“有新终端输出”点。首次加载已有日志不会把全部历史误报为未读，当前正在查看的流也不会产生自提醒，选择条目立即清除。
