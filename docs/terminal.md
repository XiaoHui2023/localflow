...
左侧列表采用事件与生命周期分离的两层语义。“运行中”组始终位于“历史”组之前，两组内部按最近时间倒序；每项只显示中性色终端图标、任务名和标签，同名任务可按标签区分。组名已经表达生命周期，不在每项重复状态文字，也不把运行中名称染成蓝色；当前项只使用轻背景与左侧强调线。宽屏列表在 248–320px 自适应，固定高度条目配合现有任务分页保持 DOM 有界。普通、完成和很久没有输出的条目都不显示圆点。任务列表 API 对每次读取实时投影当前日志文件大小；只有未选中终端的 `log_size` 在当前页面会话中真实增长，才显示“有新终端输出”信息色点。首次加载已有日志不会把全部历史误报为未读，当前正在查看的流也不会产生自提醒，选择条目立即清除。

当前 API 没有权威的最后输出时间，因此不使用任务更新时间推断“空闲很久”。若未来需要跨页面保留未读状态或显示静默时长，应先增加服务端每任务输出序列/最后输出时间与用户确认游标，再扩展 UI；不得用客户端轮询时间冒充持久事实。
