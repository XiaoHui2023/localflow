...
GNU Make 把 `name=value` 作为命令行变量覆盖，`--case` 不是 Make 的通用选项；因此验证插件不能从领域字段猜测目标程序参数语法。LocalFlow 采用按需模板合同：配置只在需要时放置 `${case}` 或 `${seed}`，也可完全不用；其它变量来自 configlib 合并后的 YAML 树。字符串由 Ubuntu shell 执行，列表保持精确 argv，插件只负责替换用户实际写出的两个插件变量并冻结。终端输出在进程启动前记录最终命令与工作目录，使 Make、脚本或原生程序的实际参数可直接核对。
