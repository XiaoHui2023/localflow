...
`custom_texts` 是供操作者核对和复制的字符串列表，不参与命令解释。每项可以引用配置变量、`${case}` 或 `${seed}`，运行配置审查区先逐行显示预览，入队事务再冻结最终值。运行配置和任务详情都不显示重复的“自定义文本”标题，只呈现文本本身；每项仍是独立复制目标，结果解析更新状态和日志字段时必须保留这些文本。
