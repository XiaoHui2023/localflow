...
`source` 也是公共执行字段，不归 command、verification 或任一具体插件所有。注册表会在插件返回任务后统一把一个路径或路径列表绑定到该任务自己的字符串命令 Shell；插件不得自行读取脚本、污染 `os.environ`、在控制器进程执行 source，或为不同任务共享可变环境。精确 argv 命令与 `source` 组合必须在配置诊断阶段拒绝。运行审查由核心逐项解析到工作目录、检查存在性并以 `code-list` 显示，插件无需重复实现。
