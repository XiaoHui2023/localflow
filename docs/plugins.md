...
`source` 也是公共执行字段，不归 command、verification 或任一具体插件所有。推荐配置是一条可直接从终端粘贴的完整 source 语句；注册表把它绑定到该任务自己的字符串命令 Shell，插件不得自行读取脚本、猜测 `-env_path` 等工具参数、污染 `os.environ`、在控制器进程执行 source，或为不同任务共享可变环境。旧的纯路径和结构化写法仍兼容。精确 argv 命令与 `source` 组合必须在配置诊断阶段拒绝。运行审查由核心显示完整语句；只有可无歧义解析的旧纯路径形式才提前检查脚本存在性。
