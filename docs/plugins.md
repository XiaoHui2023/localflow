...
`source` 也是公共执行字段，不归 command、verification 或任一具体插件所有。注册表会在插件返回任务后统一把一个路径、路径列表或 `{path, arguments}` 项绑定到该任务自己的字符串命令 Shell；插件不得自行读取脚本、猜测 `-env_path` 等工具参数、污染 `os.environ`、在控制器进程执行 source，或为不同任务共享可变环境。精确 argv 命令与 `source` 组合必须在配置诊断阶段拒绝。运行审查由核心逐项解析脚本路径到工作目录、独立引用参数、只检查脚本存在性并以 `code-list` 显示完整调用，插件无需重复实现。
