...
├── secrets/         # 受信任实例可复用的 API 与网页管理员密钥
└── config.yaml      # 启动配置
...
工作区由用户或管理员维护，可以被不同主机上的多个实例复用；实例数据目录只能由一个控制器占用，不能放在跨主机共享的 SQLite 路径。默认工作区是启动当前目录，默认数据目录是其 `.localflow/`。Case 候选是插件发现结果，不是核心工作区目录；插件可扫描自己配置的任意输入路径或从配置生成候选。任务相对工作目录仍以工作区解析，数据目录只决定历史、日志、端口、缓存与恢复身份。状态格式标记负责升级边界：旧格式可迁移或仅隔离动态内容，更高版本格式拒绝降级读取；任何恢复都不得修改配置或密钥。
...
每个任务保存显示标签与互斥键。调度器按创建序列寻找可运行任务，只有并发额度可用且任务的所有互斥键都未被运行任务占用时才启动。默认额度按服务进程的 CPU affinity、cgroup CPU 配额和系统逻辑 CPU 自动结算，也允许管理员显式覆盖；不再固定为四。大队列使用有界游标分批扫描，前方数百个互斥阻塞任务不能永久遮住后方独立任务，也不会每 500 ms 全量重扫无限队列。取得键、切换 `starting` 和记录事件在同一数据库事务完成。

批量请求先由插件展开成独立 `TaskDraft`。草稿可以声明少量由宿主拥有的自动值；存储层在同一个 SQLite 写事务中按持久化命名空间分配这些值、递归替换任务字段，然后一次写入批次、任务、事件和幂等回执。事务提交就是任务参数冻结点，调度器只改变状态和进程事实，不再修改命令或插件参数。验证仿真的每个 case、每次运行均有自己的任务 ID、递增 seed、日志和退出状态。

## 进程所有权与中断

生产环境为每个任务创建一个独立 systemd 瞬态服务和 cgroup。服务内启动轻量监督程序，由监督程序分配伪终端、记录输出、转发输入并保存退出结果。systemd 负责跨网页服务重启的进程所有权与各任务进程树边界；监督程序负责终端语义。独立 cgroup 防止生命周期、信号和子进程归属串扰，但共享服务器的 CPU、内存和 I/O 仍是有限物理资源；LocalFlow 默认不暗加资源限额，不能承诺过量并发时性能互不影响。
...
`config/` 中的任务配置是动态原始数据。文件监视器读取变化、解析并校验，成功后发布内容哈希；失败时发布诊断。网页保存必须携带所见版本，服务器将内容写入同目录临时文件，完成同步、校验和原子替换。任务配置每次展开重读文件；根目录 `config.yaml` 是独立的启动配置，只在进程启动时读取且不进入网页资源树。插件检查钩子只产生有界、只读的派生事实，不修改配置或任务。

配置加载先检查 `!include` 图不越出 `config/`，再由 `python-library-configlib` 合并。零公共任务字段保持普通配置；任一公共字段触发公共结构诊断；顶层 `plugin` 再触发插件存在性、必填字段和专属模型诊断。变量按 configlib 的合并后整树规则解析，兼容 `variables` 短名表；验证插件只延迟注入逐任务 `case` 与 `seed`，运行输入不进入变量命名空间。插件以独立 Pydantic 配置模型和输入模型完成参数校验和展开；配置资源树自动发现固定目录，不要求用户扫描。可运行文件默认直接显示插件运行参数，需要修改稳定配置时才进入编辑器。正式提交前，核心完成自动值分配和所有变量替换；解析后的命令、目录、seed、case、插件状态合同和计算字段作为不可变任务快照入队。
...
默认监听所有 IPv4 网卡并由操作系统选择端口，启动后检测首选局域网 IPv4 作为展示地址；任何来源地址都不代表用户身份。匿名访问默认只能读取去敏摘要，所有改变状态的动作都要求管理员会话。程序调用使用 `api-key` 生成请求签名；浏览器在设置页读取并输入一次只在缺失时生成的 `web-admin-key`，随后使用可跨端口与服务重启恢复的签名 cookie。可选父域 cookie 只把 HTTPS 下使用同一网页密钥的受控 DNS 兄弟节点合并为一个管理员信任边界。

摘要投影在服务端生成，不能依靠前端隐藏字段。SSE 与 WebSocket 使用同一权限判定。可信离线局域网可直接使用 HTTP；需要抵御抓包或中间人时配置 TLS。只有实际经过反向代理时才接受明确配置 CIDR 的转发头。

未登录网页无论来自回环还是远端都按匿名策略投影。服务保存权限为 `0600` 的 `web-admin-key`，但公开 `localflow` 入口不读取、不打印它，也不提供打开网页或登录子命令。登录成功后控件消失；修改秘钥文件可撤销已有浏览器会话。程序 `api-key` 与浏览器流程完全分离。
