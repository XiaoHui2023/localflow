...
默认监听所有 IPv4 网卡并由操作系统选择端口，启动后检测首选局域网 IPv4 作为展示地址；任何来源地址都不代表用户身份。匿名访问默认只能读取去敏摘要，所有改变状态的动作都要求管理员会话。程序调用使用 `api-key` 生成请求签名；浏览器在设置页读取并输入一次 `web-admin-key`，随后使用可跨服务重启恢复的签名 cookie。
