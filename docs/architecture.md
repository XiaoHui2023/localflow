...
每个任务保存显示标签与互斥键。调度器按创建序列寻找可运行任务，默认不设置 LocalFlow 并发槽位上限，未命中互斥键的任务都会被接纳启动；旧配置的 `auto` 同样按无上限处理，只有管理员显式配置整数才限流。大队列使用有界游标分批扫描，批次边界只保护控制器事件循环，不是持续并发额度；前方数百个互斥阻塞任务不能永久遮住后方独立任务，也不会每 500 ms 全量重扫无限队列。取得键、切换 `starting` 和记录事件在同一数据库事务完成。
