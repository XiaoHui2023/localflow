...
`config/` 中的任务配置是动态原始数据。文件监视器读取变化、解析并校验，成功后发布内容哈希；失败时发布诊断。网页保存必须携带所见版本，服务器将内容写入同目录临时文件，完成同步、校验和原子替换。任务配置每次展开重读文件；根目录 `localflow.yaml` 是独立的启动配置，只在进程启动时读取且不进入网页资源树。插件检查钩子只产生有界、只读的派生事实，不修改配置或任务。
