...
配置根由用户或管理员维护，可以被不同主机上的多个实例复用；实例状态根只能由一个控制器占用，不能放在跨主机共享的 SQLite 路径。默认配置根是启动当前目录，默认状态根是其 `.localflow/`。任务相对工作目录仍以配置根解析，状态根只决定历史、日志、端口、缓存与恢复身份。状态格式标记负责升级边界：旧格式可迁移或仅隔离动态内容，更高版本格式拒绝降级读取；任何恢复都不得修改配置或密钥。
