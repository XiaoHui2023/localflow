...
中断按阶段推进：先向前台进程组发送 `SIGINT`，宽限期结束仍存活时发送 `SIGTERM`，最后发送 `SIGKILL`。任务行记录当前阶段，事件流记录阶段变化。网页只有一个中断入口，重复操作返回已有中断序列，不并行发送第二套信号。

开发执行器直接创建新会话与进程组，用于 Windows 开发和 Linux 自动测试。它不能作为 Ubuntu 生产部署已验证的替代品。

## 数据与实时通道

SQLite 保存任务快照、事件、确认状态、幂等结果和批次。WAL 允许读取与短写事务并行。日志正文按任务写入磁盘，数据库保存最终大小，详情接口根据固定根目录计算日志位置。

任务状态使用服务器发送事件广播，并带单调递增事件序号供重连续传。交互终端使用 WebSocket；只读角色只能接收输出，管理员才可发送输入和调整尺寸，非法输入返回 `error` 消息。

## 配置更新

磁盘文件是原始数据。文件监视器读取变化、解析并校验，成功后发布内容哈希；失败时发布诊断且不会改变已加载的进程级设置。网页保存必须携带所见版本，服务器将内容写入同目录临时文件，完成同步、校验和原子替换。声明式模板每次展开重读磁盘；`server.yaml` 的进程级设置在重启后生效。

变量按全局、项目、模板、本次运行合并。插件在任务建立前完成参数校验和展开；解析后的命令、目录、seed、case 和计算字段写入不可变任务快照。
