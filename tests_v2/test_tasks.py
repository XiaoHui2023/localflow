...
def test_source_contract_requires_a_non_empty_path_list(
    admin: TestClient, root: Path
) -> None:
    base = {
    ...
    for invalid in (
        ...
        [],
        [""],
        ["scripts/setup.csh", {"path": "scripts/second.csh"}],
    ):
        response = admin.post("/api/v1/tasks", json={**base, "source": invalid})
