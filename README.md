...
默认绑定 `127.0.0.1`，端口为 `0`。实际访问地址会打印到终端并在运行期间写入 `runtime/port`。来源地址和随机端口都不是身份验证；未登录网页默认只能读取去敏摘要。首次管理操作在设置页输入 `secrets/web-admin-key`，成功后浏览器保持登录，刷新或服务重启无需重输；修改该文件会注销旧会话。程序客户端使用独立的 `secrets/api-key` 逐请求 HMAC 签名。
