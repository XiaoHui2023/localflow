...
Hosted run `34710324346` proved the source, systemd/PTY/cgroup, glibc 2.23 build, and legacy CPU stages, then correctly stopped at the frozen smoke because that checker still expected the hidden `--state-dir` alias in public help. The checker and both browser launchers now exercise `--workspace`/`--data`; the frozen checker also rejects accidental re-exposure of either legacy spelling.
