...
async function waitForState(page, taskId, states) {
  await expect.poll(
    async () => (await api(page, `/tasks/${taskId}`)).state,
    { timeout: 20_000 },
  ).toMatch(new RegExp(`^(${states.join("|")})$`));
}

test("Ubuntu browser can operate the released web console", async ({ page, browser, browserName }, testInfo) => {
  const errors = [];
  ...
  page.on("response", (response) => {
    if (response.url().endsWith("/api/v1/events") && response.status() === 200) eventResponses += 1;
  });
  page.on("requestfailed", (request) => {
    const reason = request.failure()?.errorText || "unknown failure";
    const expectedEventClose = request.url().endsWith("/api/v1/events") && /ABORT/i.test(reason);
    if (!expectedEventClose) failedRequests.push(`${request.method()} ${request.url()}: ${reason}`);
  });
  await page.addInitScript(() => {
    const original = Document.prototype.execCommand;
    Document.prototype.execCommand = function execCommand(command, ...args) {
      if (command === "copy") {
        window.__localflowCopiedText = document.activeElement?.value || window.getSelection()?.toString();
        return true;
      }
  ...
  await expect(page.locator('[data-file="config/command/hello-world.yaml"]')).toBeVisible();
  ...
  const finished = await api(page, "/tasks", {
    ...
    body: {
      ...
      command: [process.env.LOCALFLOW_QA_PYTHON, "-c", "print('browser-compat-ok', flush=True)"],
  ...
  const finishedRow = page.getByRole("button", { name: new RegExp(`${browserName}-compat-finished`) });
  await finishedRow.click();
  const copyValue = page.locator(".task-item.open .copy-value").filter({ hasText: "qa://ubuntu-browser" });
  await copyValue.click();
  await expect(copyValue.locator("..")).toHaveAttribute("data-copied", "true");
  expect(await page.evaluate(() => window.__localflowCopiedText)).toBe("qa://ubuntu-browser");
  ...
  const live = await api(page, "/tasks", {
    ...
    body: {
      ...
      working_directory: process.env.LOCALFLOW_QA_ROOT,
      command: [process.env.LOCALFLOW_QA_PYTHON, "-c", "import time; print('terminal-compat-ready', flush=True); time.sleep(30)"],
      labels: ["browser-compat"],
    },
  });
  await waitForState(page, live.task_id, ["running"]);
  await page.getByRole("tab", { name: "终端" }).click();
  await expect(page.locator(".terminal-page .xterm-rows")).toContainText("terminal-compat-ready", { timeout: 20_000 });
  ...
  if (evidence) {
    ...
    fs.writeFileSync(path.join(evidence, `${testInfo.project.name}.json`), JSON.stringify({
      result: "passed",
