...
async function runAcceptance(page) {
  ...
  const helloConfig = page.locator('[data-file="config/command/hello-world.yaml"]');
...
test("plugin configuration console remains concise and operable in Edge", async ({ page, browser }) => {
  ...
  let openWebSockets = 0; let terminalAcks = 0;
  page.on("websocket", (socket) => { openWebSockets += 1; socket.on("framesent", ({ payload }) => { try { if (JSON.parse(String(payload)).type === "ack") terminalAcks += 1; } catch { /* binary/user frames are not protocol JSON */ } }); socket.on("close", () => { openWebSockets -= 1; }); });
  ...
  const simulation = await browserApi(page, "/config/files/verification/demo.yaml/runs", { method: "POST", body: { inputs: { cases: ["case-a"], case_runs: { "case-a": 1 }, seed: "" } } });
  ...
  await page.getByRole("tab", { name: "运行" }).click(); await expect(page.locator(".tree-node").first()).toBeVisible(); await expect(page.locator(".config-workbench")).toBeVisible();
  ...
  const resourceMetrics = { ...JSON.parse(process.env.LOCALFLOW_QA_SERVER_RESOURCES), ...await measureWebResources(page, () => openWebSockets), terminal_ack_frames: terminalAcks };
  assertResourceBudget(resourceMetrics);
  await expect(page.locator('[data-file="config/shared/qa-defaults.yaml"]')).toHaveAttribute("data-config-state", "fragment");
  await expect(page.locator('[data-file="config/command/hello-world.yaml"]')).toHaveAttribute("data-config-state", "task");
  await expect(page.locator('[data-file="config/shared/qa-defaults.yaml"]')).toHaveAttribute("aria-label", /共享片段/);
  await expect(page.locator('[data-file="config/command/qa-invalid.yaml"]')).toHaveAttribute("data-config-state", "invalid");
  await expect(page.locator('[data-file="config/command/qa-invalid.yaml"]')).toHaveAttribute("aria-label", /配置有误/);
  const filenameColors = await page.locator('[data-file="config/shared/qa-defaults.yaml"]>span:last-child, [data-file="config/command/hello-world.yaml"]>span:last-child, [data-file="config/command/qa-invalid.yaml"]>span:last-child').evaluateAll((items) => items.map((item) => getComputedStyle(item).color)); expect(new Set(filenameColors).size).toBe(1);
  await page.getByRole("button", { name: "新建文件" }).click(); await page.getByLabel("名称").fill("qa-command"); await page.getByRole("dialog").locator("select").selectOption("command"); await page.getByRole("button", { name: "创建" }).click(); await expect(page.locator('[data-file="config/command/qa-command.yaml"]')).toBeVisible();
  await page.getByRole("button", { name: "重命名" }).click(); const rename = page.locator(".tree-node input"); await expect(rename).toBeVisible(); await rename.fill("qa-renamed"); await rename.press("Enter"); await expect(page.locator('[data-file="config/command/qa-renamed.yaml"]')).toBeVisible();
  ...
  await page.locator('[data-file="folder:plugins"]').click(); await page.getByRole("button", { name: "新建目录" }).click(); await page.getByLabel("名称").fill("qa-tools"); await page.getByRole("button", { name: "创建" }).click(); expect(fs.statSync(path.join(qaRoot, "plugins", "qa-tools")).isDirectory()).toBeTruthy();
  await expect(page.locator('[data-file="plugins/command.py"]')).toHaveAttribute("data-config-state", "plugin"); await expect(page.locator('[data-file="plugins/command.py"]')).toHaveAttribute("aria-label", /插件文件/); await expect(page.locator('[data-file="folder:plugins/__pycache__"]')).toHaveCount(0);
  await page.locator('[data-file="folder:plugins"]').click(); await page.getByRole("button", { name: "新建目录" }).click(); await page.getByLabel("名称").fill("qa-dest"); await page.getByRole("button", { name: "创建" }).click();
  await page.locator('[data-file="folder:plugins/qa-tools"]').click(); await page.getByRole("button", { name: "新建文件" }).click(); await page.getByLabel("名称").fill("qa-agent"); await page.getByRole("button", { name: "创建" }).click(); await page.waitForTimeout(1200); const pluginEditor = page.locator(".monaco-editor").first(); await pluginEditor.click({ position: { x: 80, y: 60 } }); await page.keyboard.insertText("VALUE = 1"); await expect(pluginEditor.locator(".view-lines")).toContainText("VALUE = 1"); await page.getByRole("button", { name: "保存", exact: true }).click(); await expect(page.getByRole("status")).toContainText("已保存"); expect(fs.readFileSync(path.join(qaRoot, "plugins", "qa-tools", "qa-agent.py"), "utf8")).toContain("VALUE = 1");
  await page.getByRole("button", { name: "复制", exact: true }).click(); await page.locator('[data-file="folder:plugins"]').click(); await page.getByRole("button", { name: "粘贴", exact: true }).click(); await expect(page.locator('[data-file="plugins/qa-agent.py"]')).toBeVisible();
  await page.locator('[data-file="plugins/qa-agent.py"]').click(); await page.locator(".config-explorer").press("ControlOrMeta+x"); await page.locator('[data-file="folder:plugins/qa-dest"]').click(); await page.locator(".config-explorer").press("ControlOrMeta+v"); await expect(page.locator('[data-file="plugins/qa-dest/qa-agent.py"]')).toBeVisible(); await expect(page.locator('[data-file="plugins/qa-agent.py"]')).toHaveCount(0);
  fs.writeFileSync(path.join(qaRoot, "plugins", "qa-dest", "qa-agent.py"), "VALUE = 2\n", "utf8"); await page.locator('[data-file="plugins/qa-dest/qa-agent.py"]').click(); await expect(page.getByRole("status")).toContainText("已同步外部修改", { timeout: 5_000 }); await expect(page.locator(".monaco-editor .view-lines")).toContainText("VALUE = 2");
  await page.locator('[data-file="config/command/qa-invalid.yaml"]').click(); await expect(page.locator(".config-diagnosis")).toContainText("labels"); await expect(page.getByRole("button", { name: "运行", exact: true })).toHaveCount(0);
  await page.locator('[data-file="config/shared/qa-defaults.yaml"]').click(); await expect(page.getByRole("button", { name: "运行", exact: true })).toHaveCount(0);
  await page.locator('[data-file="config/verification/demo.yaml"]').click(); await expect(page.getByText("smoke", { exact: true })).toBeVisible(); await expect(page.getByText("case-a", { exact: true })).toBeVisible(); await expect(page.locator(".inspection-item")).toContainText(["工作目录", "命令", "Case 目录", "脚本"]); await expect(page.locator(".inspection-item.severity-error")).toHaveCount(0); await expect(page.getByLabel("搜索 Case")).toHaveCount(0); await expect(page.locator(".case-count")).toHaveCount(0); await expect(page.getByRole("button", { name: "运行", exact: true })).toBeDisabled(); await expect(page.getByLabel("随机种子")).toHaveValue(""); await expect(page.getByText(/\.ya?ml|\.json|\.toml/i)).toHaveCount(0); const caseListBox = await page.locator(".case-list").boundingBox(); const caseBoxes = await page.locator(".case-item").evaluateAll((nodes) => nodes.map((node) => { const box = node.getBoundingClientRect(); return { width: box.width, height: box.height, x: box.x, y: box.y }; })); expect(caseBoxes.every((box) => box.height <= 40 && box.width / caseListBox.width >= 0.95)).toBeTruthy(); expect(new Set(caseBoxes.map((box) => Math.round(box.x))).size).toBe(1); expect(new Set(caseBoxes.map((box) => Math.round(box.y))).size).toBe(caseBoxes.length); const firstCaseMain = page.locator(".case-main").first(); const idleCaseBorder = await page.locator(".case-item").first().evaluate((node) => getComputedStyle(node).borderColor); await firstCaseMain.focus(); const focusedCaseBorder = await page.locator(".case-item").first().evaluate((node) => getComputedStyle(node).borderColor); expect(focusedCaseBorder).not.toBe(idleCaseBorder); await expect(page.locator(".case-count")).toHaveCount(0); const caseTransitions = await page.locator(".case-item").first().evaluate((node) => getComputedStyle(node).transitionDuration.split(",").map((value) => Number.parseFloat(value) * (value.includes("ms") ? 0.001 : 1))); expect(Math.max(...caseTransitions)).toBeLessThanOrEqual(0.1); await page.screenshot({ path: path.join(evidence, "admin-run-verification-empty-dark.png"), fullPage: true });
  ...
  await page.getByRole("tab", { name: "设置" }).click(); await page.getByRole("tab", { name: "运行" }).click(); await expect(page.locator('[data-file="config/verification/demo.yaml"]')).toHaveClass(/selected/); await expect(page.locator('[data-case="case-b"] .case-count')).toHaveText("×2");
  ...
  await page.locator('[data-file="config/generic-picker/demo.yaml"]').click(); await expect(page.getByText("case-a", { exact: true })).toBeVisible(); await page.getByRole("button", { name: "case-a，未运行" }).click(); await page.getByRole("button", { name: "case-a，1 次" }).click(); await expect(page.locator('[data-case="case-a"] .case-count')).toHaveText("×2"); const genericRunResponse = page.waitForResponse((response) => response.url().endsWith("/api/v1/config/files/generic-picker/demo.yaml/runs") && response.request().method() === "POST"); await page.getByRole("button", { name: "运行", exact: true }).click(); expect((await genericRunResponse).json()).resolves.toMatchObject({ count: 2 });
  await page.locator('[data-file="config/command/hello-world.yaml"]').click(); await expect(page.getByRole("button", { name: "编辑" })).toBeVisible(); const headerBeforeNotice = await page.locator(".workbench-header").boundingBox(); const explorerBeforeNotice = await page.locator(".explorer").boundingBox(); await page.getByRole("button", { name: "运行", exact: true }).click(); const configNotice = page.getByText(/已加入 1 个任务/); await expect(configNotice).toBeVisible(); await expect(configNotice).toHaveCSS("position", "fixed"); expect(await page.locator(".workbench-header").boundingBox()).toEqual(headerBeforeNotice); expect(await page.locator(".explorer").boundingBox()).toEqual(explorerBeforeNotice); await expect(configNotice).toHaveCount(0, { timeout: 5_000 });
  await page.screenshot({ path: path.join(evidence, "admin-config-explorer-dark.png"), fullPage: true });
  await page.getByRole("tab", { name: "任务" }).click(); await expect(page.getByRole("button", { name: /hello-world/ })).toBeVisible({ timeout: 20_000 });
