...
test("plugin configuration console remains concise and operable in Edge", async ({ page, browser }) => {
  ...
  await expect(page.locator('[data-file="shared/qa-defaults.yaml"]')).toHaveAttribute("data-config-state", "fragment");
  await expect(page.locator('[data-file="command/hello-world.yaml"]')).toHaveAttribute("data-config-state", "task");
  await expect(page.locator('[data-file="shared/qa-defaults.yaml"]')).toHaveAttribute("aria-label", /共享片段/);
  await expect(page.locator('[data-file="command/qa-invalid.yaml"]')).toHaveAttribute("data-config-state", "invalid");
  await expect(page.locator('[data-file="command/qa-invalid.yaml"]')).toHaveAttribute("aria-label", /配置有误/);
  const filenameColors = await page.locator('[data-file="shared/qa-defaults.yaml"]>span:last-child, [data-file="command/hello-world.yaml"]>span:last-child, [data-file="command/qa-invalid.yaml"]>span:last-child').evaluateAll((items) => items.map((item) => getComputedStyle(item).color)); expect(new Set(filenameColors).size).toBe(1);
  await page.getByRole("button", { name: "新建" }).click(); await page.getByLabel("名称").fill("qa-command"); await page.getByLabel("插件").selectOption("command"); await page.getByRole("button", { name: "创建" }).click(); await expect(page.locator('[data-file="command/qa-command.yaml"]')).toBeVisible();
  await page.getByRole("button", { name: "重命名" }).click(); const rename = page.locator(".tree-node input"); await expect(rename).toBeVisible(); await rename.fill("qa-renamed"); await rename.press("Enter"); await expect(page.locator('[data-file="command/qa-renamed.yaml"]')).toBeVisible();
  ...
  await page.getByRole("tab", { name: "设置" }).click(); await page.getByRole("tab", { name: "运行" }).click(); await expect(page.locator('[data-file="verification/demo.yaml"]')).toHaveClass(/selected/); await expect(page.locator('[data-case="case-b"] .case-count')).toHaveText("×2");
