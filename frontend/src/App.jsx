...
function Config({ theme }) {
  ...
  const [files, setFiles] = useState([]); const [diagnostics, setDiagnostics] = useState({}); const [plugins, setPlugins] = useState([]); const [file, setFile] = useState(); const [content, setContent] = useState(""); const [mode, setMode] = useState("edit"); const [values, setValues] = useState({}); const [notice, setNotice] = useState(""); const [runStatus, setRunStatus] = useState("idle"); const [conflict, setConflict] = useState(); const [createOpen, setCreateOpen] = useState(false); const [createPath, setCreatePath] = useState(""); const [createPlugin, setCreatePlugin] = useState(""); const [deleting, setDeleting] = useState(false); const [treeOpen, setTreeOpen] = useState(false); const [treeSize, setTreeSize] = useState({ width: 260, height: 600 }); const treeHost = useRef();
  ...
  const move = async (source, target) => { const current = file?.path === source ? file : await api.file(source); const moved = await api.moveFile(source, target, current.version); localVersionsRef.current.set(moved.path, moved.version); await reload(); await open(moved.path); setNotice("已移动"); };
  ...
  const save = async () => { try { const saved = await api.saveFile(file.path, content, file.version); localVersionsRef.current.set(saved.path, saved.version); await open(saved.path, false); setNotice("已保存"); } catch (error) { if (error.status === 412) { setConflict(await api.file(file.path)); setNotice("文件已变化，请比较后选择"); } else setNotice(`保存失败：${error.message}`); } };
  const run = async () => { setRunStatus("submitting"); setNotice(""); try { const result = await api.runConfig(file.path, pluginInputs(selectedPlugin, values)); setRunStatus("accepted"); setNotice(`已加入 ${result.count} 个任务`); } catch (error) { setRunStatus("idle"); setNotice(`运行失败：${error.message}`); } };
  const create = async () => { try { const named = /\.(?:ya?ml|json|toml)$/i.test(createPath) ? createPath : `${createPath}.yaml`; const path = named.includes("/") ? named : `${createPlugin}/${named}`; const saved = await api.createFile(path, createPlugin); localVersionsRef.current.set(saved.path, saved.version); setCreateOpen(false); setCreatePath(""); await reload(); await open(saved.path, false); setNotice("已创建"); } catch (error) { setNotice(`创建失败：${error.message}`); } };
