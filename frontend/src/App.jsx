...
function pluginInputs(plugin, values) {
  ...
  return Object.fromEntries([...names].filter((name) => Object.hasOwn(values, name)).map((name) => [name, values[name]]));
}
...
function Config({ theme }) {
  ...
  useEffect(() => { const events = new EventSource("/api/v1/events"); const changed = (event) => { const data = JSON.parse(event.data); reload(); if (data.path === file?.path) { open(file.path); setNotice("已同步外部修改"); } }; events.addEventListener("config.changed", changed); events.addEventListener("config.invalid", (event) => { const data = JSON.parse(event.data); if (data.path === file?.path) setNotice(`配置无效：${data.error}`); }); return () => events.close(); }, [file?.path, reload, open]);
  ...
  const onMove = async ({ dragIds, parentId }) => { const targetFolder = parentId?.replace(/^folder:/, "") || ""; for (const id of dragIds.filter((item) => !item.startsWith("folder:"))) await move(id, `${targetFolder ? `${targetFolder}/` : ""}${id.split("/").at(-1)}`); };
  ...
  const create = async () => { try { const named = /\.(?:ya?ml|json|toml)$/i.test(createPath) ? createPath : `${createPath}.yaml`; const path = named.startsWith("tasks/") ? named : `tasks/${named}`; const saved = await api.createFile(path, createPlugin); setCreateOpen(false); setCreatePath(""); await reload(); await open(saved.path, false); setNotice("已创建"); } catch (error) { setNotice(`创建失败：${error.message}`); } };
...
function SettingsPage({ theme, setTheme }) {
  ...
  return <div className="settings"><section className="settings-panel"><div className="setting-row"><span>主题</span><div className="theme-choice" role="group" aria-label="主题"><button className={theme === "light" ? "active" : ""} aria-pressed={theme === "light"} onClick={() => setTheme("light")}><Sun/>浅色</button><button className={theme === "dark" ? "active" : ""} aria-pressed={theme === "dark"} onClick={() => setTheme("dark")}><Moon/>深色</button></div></div><label className="setting-row"><span>时间校准</span><span className="time-control"><input aria-label="时间校准" type="datetime-local" step="1" value={displayedTime} onFocus={() => { setReferenceTime(displayedTime); setEditing(true); }} onChange={(event) => { setReferenceTime(event.target.value); setDirty(true); }}/>{notice && <small>{notice}</small>}</span></label></section></div>;
}

let bootstrapExchange;
export default function App({ bootstrapCode }) {
  useUiRevision();
  const [page, setPage] = useState("tasks"); const [status, setStatus] = useState(); const [tasks, setTasks] = useState([]); const [opened, setOpened] = useState(); const [fresh, setFresh] = useState(new Set()); const [error, setError] = useState(""); const [bootstrapError, setBootstrapError] = useState(""); const [theme, setTheme] = useTheme();
  ...
  useEffect(() => { if (!bootstrapCode || !status || status.role === "admin") return; if (!bootstrapExchange) bootstrapExchange = api.login(bootstrapCode); bootstrapExchange.then(refresh).catch(() => setBootstrapError("管理员会话未建立，请重新运行 localflow open。")); }, [bootstrapCode, status, refresh]); useEffect(() => { if (status && status.role !== "admin" && page !== "tasks") setPage("tasks"); }, [status, page]); useEffect(() => { setFresh(new Set(tasks.filter((task) => task.newly_completed).map((task) => task.id))); }, [tasks]);
  ...
  return <div className="app-shell"><aside className="top"><nav role="tablist" aria-label="主导航" aria-orientation="vertical">{navigation.map(([id, text, Icon], index) => <button id={`nav-${id}`} role="tab" aria-selected={page === id} aria-controls="page-panel" tabIndex={page === id ? 0 : -1} className={page === id ? "active" : ""} onClick={() => setPage(id)} onKeyDown={(event) => moveNavigation(event, index)} key={id}><Icon/><span>{text}</span></button>)}</nav></aside><div className="app-content">{(error || bootstrapError) && <p className="global-error">{bootstrapError || error}</p>}<main id="page-panel" role="tabpanel" aria-labelledby={`nav-${page}`} tabIndex="0">{page === "tasks" && <section className="workspace">{definitions.length === 0 ? <div className="tasks-empty"><ListChecks/><p>暂无任务</p></div> : definitions.map(([key, title, Icon]) => <section className="group" key={key}><h2><Icon/>{title}<span>{groups[key].length}</span></h2><div>{key === "queued" ? <QueueTasks tasks={groups[key]} renderTask={(task, count) => renderTask(key, task, count)}/> : groups[key].map((task) => renderTask(key, task))}</div></section>)}</section>}{status?.role === "admin" && <><div hidden={page !== "config"}><Config theme={theme}/></div>{page === "terminal" && <TerminalPage tasks={tasks} role={status?.role} theme={theme}/>} {page === "settings" && <SettingsPage theme={theme} setTheme={setTheme}/>}</>}</main></div></div>;
}
